...
if (isset($_POST['simpan'])) {
    ...
    if ($id_barang > 0 && !empty($tanggal) && $jumlah > 0) {
        ...
        if ($hasil === true) {
            echo "<script>alert('Transaksi barang masuk berhasil dicatat!'); window.location='index.php';</script>";
        } else {
            echo "<div class='alert alert-danger'>Gagal: " . $hasil . "</div>";
        }
    }
}
?>
    ...
    <div class="col-md-6">
        <div class="card shadow-sm border-0">
            <div class="card-body p-4">
                <h4 class="card-title mb-4 fw-bold text-success">Input Barang Masuk (Restock)</h4>
                <form action="" method="POST">
                    <div class="mb-3">
                        ...
                        <select name="id_barang" class="form-select" required>
                            ...
                            <?php foreach ($dataBarang as $brg): ?>
                                <option value="<?= $brg['id_barang']; ?>"><?= htmlspecialchars($brg['nama_barang']); ?> (Stok saat ini: <?= $brg['stok']; ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="d-flex justify-content-between mt-4">
                        ...
                        <button type="submit" name="simpan" class="btn btn-success btn-sm">Simpan Transaksi</button>
                    </div>
                </form>
            </div>
        </div>
